...
syn match   knkPrim       '\%(call\|apply\%(-dict\)\?\|if\|def\|def\%(multi\|record\)\|=>\|dict\|show\|puts!\|say!\|ask!\|type\|callable?\|function?\|defmodule\|import\%(-from\)\?\|=\|not=\|[<>]=\?\|n\?eq\|[lg]te\?\|cmp\|abs\|trunc\|round\|ceil\|floor\|int->float\|record->dict\|record-\%(type\%(-\%(name\|fields\)\)\?\|values\)\|fail\|try\|rx-\%(match\|sub\)\|par\|sleep\|__[^, \t]\+__\)\%(\[\?\%([, \t]\|$\)\)\@='
